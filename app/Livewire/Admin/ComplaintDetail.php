...
/**
 * The complaint-resolution actions (verdict, chat, penalty-access restore)
 * also live on the general order detail page (Admin\OrderDetail) -
 * duplicated here deliberately, not extracted/shared, so an admin working
 * the Complaints queue can resolve one without leaving it for the general
 * Orders list. Admin\OrderDetail and /admin/orders are left untouched.
 */
...
class ComplaintDetail extends Component
{
    ...
    {
        abort_if($order->complaint_status === null, 404);
    ...
    private function refreshOrder(): void
    {
        $this->order->refresh()->load([
            'buyer', 'seller', 'items.product', 'auction',
