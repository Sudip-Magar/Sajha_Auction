<div x-data class="min-h-[calc(100vh-64px)] bg-linear-to-br from-[#1F6F5F] to-[#2FA084] py-8 px-4 flex items-center justify-center">
    <div class="w-full max-w-5xl bg-white/95 backdrop-blur-xl rounded-[2rem] shadow-2xl overflow-hidden border border-white/20">
        ...
        <div class="flex flex-col lg:flex-row">
            ...
            <div class="w-full lg:w-[320px] bg-linear-to-br from-[#1F6F5F] to-[#2FA084] p-8 text-white flex flex-col justify-center relative overflow-hidden">
                ...
                {{-- Avatar Upload Card --}}
                <div class="relative z-10 bg-white/10 backdrop-blur-md border border-white/20 rounded-2xl p-6 text-center shadow-xl">
                    <div class="relative inline-block mb-4 group">
                        @if ($avatarFile)
                            <img src="{{ $avatarFile->temporaryUrl() }}" class="w-24 h-24 rounded-full object-cover ring-4 ring-white/30 mx-auto transition group-hover:scale-105" alt="Avatar" />
                        @elseif ($googleAvatar)
                            <img src="{{ $googleAvatar }}" class="w-24 h-24 rounded-full object-cover ring-4 ring-white/30 mx-auto transition group-hover:scale-105" alt="Avatar" />
                        @else
                            <img src="{{ asset('assets/images/user.png') }}" class="w-24 h-24 rounded-full object-cover ring-4 ring-white/30 mx-auto transition group-hover:scale-105" alt="Avatar" />
                        @endif
                        <label for="avatarUpload" class="absolute bottom-1 right-1 w-8 h-8 bg-white text-[#1F6F5F] rounded-full flex items-center justify-center shadow-lg cursor-pointer hover:scale-110 transition active:scale-95">
            ...
            <div class="flex-1 p-8 lg:p-10">
                <div class="space-y-5">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <x-input label="Full Name" wire:model="name" icon="o-user-circle" placeholder="Your full name" class="bg-gray-50/50 border-gray-200" />
                        <x-input label="Username" wire:model.live.debounce.500ms="username" icon="o-at-symbol" placeholder="username" class="bg-gray-50/50 border-gray-200" />
                    </div>
                    ...
                    {{-- Row 3: Bio & Gender --}}
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <x-select label="Gender" icon="o-user" :options="$genderStates" wire:model="gender" placeholder="Select Gender" class="bg-gray-50 border-gray-200" />
                        <x-textarea label="Short Bio" wire:model="bio" rows="2" placeholder="Tell us about yourself..." class="bg-gray-50/50 border-gray-200 resize-none" />
                    </div>
                    ...
                    <div x-data="{
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <x-password label="Password" wire:model.live="password" placeholder="New Password" class="bg-gray-50/50 border-gray-200" />
                            <x-password label="Confirm Password" wire:model.live="confirm_password" placeholder="Repeat Password" class="bg-gray-50/50 border-gray-200" />
                        </div>

                        {{-- Compact Password Feedback --}}
                        <div class="flex flex-wrap items-center gap-3">
                            <template x-for="rule in rules">
                                <div class="flex items-center gap-1.5 transition-all duration-200" :class="rule.check() ? 'text-green-600' : 'text-gray-400'">
                                    <div class="w-3.5 h-3.5 rounded-full flex items-center justify-center border" :class="rule.check() ? 'bg-green-500 border-green-500 text-white' : 'bg-white border-gray-200 text-gray-200'">
                            ...
                            <div x-show="cpw && cpw.length > 0" class="flex items-center gap-1.5 ml-auto">
                                <span class="text-[10px] font-bold" :class="passwordsMatch ? 'text-green-600' : 'text-red-500'" x-text="passwordsMatch ? 'Passwords Match' : 'Mismatch'"></span>
                            </div>
                        </div>
                    </div>
